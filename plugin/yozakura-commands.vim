...
command! YozakuraDefault lua require('yozakura').set_palette(nil)
...
function! s:YozakuraCompletePalette(ArgLead, CmdLine, CursorPos)
  return ['default', 'soft_contrast', 'warm_gray', 'muted_rose']
endfunction

command! -nargs=? -complete=customlist,s:YozakuraCompletePalette YozakuraPalette 
  \ lua require('yozakura').set_palette(<q-args> == 'default' and nil or <q-args>)